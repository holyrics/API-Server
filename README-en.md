...
| `params.*.default_value` | _String (optional)_ | Default/initial value of the variable |
| `params.*.only_number` | _Boolean (optional)_ | Accept only numbers |
| `params.*.uppercase` | _Boolean (optional)_ | Accept only uppercase characters |
...
| `id` | _String (optional)_ | Item ID |
| `index` | _Number (optional)_ | Item position in the list (starts at zero). |
| `midi_time_code` | _Number_ | Time in milliseconds |
...
| Name | Type  | Description |
| ---- | :---: | ------------|
| `module_id` | _String_ | module id |
| `action_id` | _String_ | action ID |
| `action_type` | _String (optional)_ | Accepted values: `call` `open`<br>**call:** Performs the action<br>**open:** Abre uma janela popup para o usuário inserir/editar os parâmetros e executar a ação `Default: call` |
| `async` | _Boolean (optional)_ | Execute the action asynchronously, that is, without returning the response of the action. Available if `action_type=call` `Default: false` |
| `timeout` | _Number (optional)_ | `100 ~ 4000` Timeout for action execution. Available if `action_type=call` `Default: 500` |
| `notification` | _Boolean (optional)_ | Exibir uma notificação em vez de exibir o popup diretamente na tela para o usuário. Available if `action_type=open` `Default: false` |
| `input` | _Object (optional)_ | Key/value map with the parameters for executing the action, where each key is the respective id of the input declared in the module's action |
...
| `merge` | _Boolean_ | Adds the items to the end of the list instead of replacing `Default: false` `v2.24.0+` |
...
| `jscommunity_id` | _String (optional)_ |  |
| `info_id` | _String (optional)_ |  |
| `active` | _Boolean (optional)_ |  |
| `enabled_by_user` | _Boolean (optional)_ |  |
| `conditional_execution` | _Boolean (optional)_ |  |
...
| Name | Type  | Description |
| ---- | :---: | ------------|
| `file` | _String (optional)_ | File location in the **Images** tab<br>Starting from `v2.29.0`, it is possible to use the syntax `background:id` in this `file` parameter, where `id` is the ID of the theme or background that will be used as the wallpaper image. Example: 'background:1234' |
...
| Name | Type  | Description |
| ---- | :---: | ------------|
| `type` | _Object_ | Preset type. Accepted values: `public` `return` `image` `html` |
...
| ---- | :---: | ------------|
| `id` | _String_ | Rule group ID |
| `rule_id` | _String (optional)_ | ID of the specific rule to be tested instead of testing the entire group |
...
| Name | Type  | Description |
| ---- | :---: | ------------|
| `event_id` | _String_ | Event ID Can be obtained from a **Schedule** object |
